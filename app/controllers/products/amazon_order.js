...
export function getOrders(userid, userkeys, callback) {
...
async.eachSeries(marketplaceIds, function (marketplace, next) {
    ...
    let params = {
        ...
        ReportType: "_GET_FLAT_FILE_ALL_ORDERS_DATA_BY_LAST_UPDATE_",
         StartDate:'2018-02-01T07:00:01+00:00',
      EndDate:'2018-02-03T07:00:52+00:00',
        SellerId: sellerID,
        ...
        callback: function(error, response, body) {
            ...
            converter.fromString(body, function(err, resultfinal) {
                ...
                async.each(resultfinal, function(single, callbacks) {
                  ...
                  //var id=single["amazon-order-id"];
                  //console.log(id);
                    var orders = new Orders();
                     orders.orderlist.amazon_order_id=single["amazon-order-id"];
                    orders.orderlist.merchant_orderid=single["merchant-order-id"];
                    orders.orderlist.purchase_date=single["purchase-date"];
                    orders.orderlist.sales_channel=single["sales-channel"];
                    orders.orderlist.sku=single["sku"];
                    orders.orderlist.asin=single["asin"];
                    orders.orderlist.order_status=single["order-status"];
                    orders.orderlist.quantity=single["quantity"];
                    orders.orderlist.currency=single["currency"];
                    orders.orderlist.userid=userid;
                    orders.save(function(err, responses) {
                                if (err) {
                                    console.log(err);
                                }
                        ...
                        if (resultprod.length > 0) {
                           ...
                           Products.findByIdAndUpdate(resultprod[0]._id,{ 'productlist.seller_sku': sku, 'productlist.asin': asin, 'productlist.item_name': item_name, 'productlist.item_descp': item_description, 'productlist.price': price, 'productlist.quantity': quantity, 'productlist.pending_quantity': pending_quantity, 'productlist.product_id': product_id, 'productlist.fulfillment_channel': fulfillment_channel, 'productlist.merchant_shipping_group': merchant_shipping_group, 'productlist.listing_id': listing_id, 'productlist.open_date': open_date, 'productlist.status': status }, function(err, resultup) {
                               if (err) {
                                    ...
                                    callbacks(nu products_detail.productlist.seller_sku = sku;ll);
                                }
